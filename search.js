...
function display() {
	...
	if (isSearchingRelatedVideos && pageIndices[relatedVideoIDs.length] == 1) {
		element.innerHTML = "<div id='lastPage' style='opacity:0' " +
	...
	for (var video = 0; video < videoIDs.length; video++) {
		...
		element.innerHTML = element.innerHTML +
			"<div style='opacity:0' class='video' id='video" + video + "'><h2 title='" + title +
			"'>" + title + "</h2>" + "<img class='videoPic' id='videoPic" + video +
			"' onclick='videoIndex=" + video + ";change(\"" + id + "\");' " +
...
function search() {
	...
	if (playlistMatchArray) { // if is not null (has elements)
...
function searchRelatedVideos() {
	...
	relatedVideoIDs.push(player.getVideoUrl().match(/v=.........../g)[0].substring(2));
		...
		return $.ajax('https://www.googleapis.com/youtube/v3/search?pageToken=' +
			...
			{type: 'GET',
				...
				success: function (data, status, jqxhr) {
					...
					currPageTokens[relatedVideoIDs.length] = nextPageTokens[relatedVideoIDs.length];
	...
	if (isSearchingRelatedVideos) {
		return $.ajax('https://www.googleapis.com/youtube/v3/search?pageToken=' +
			...
			{type: 'GET',
				...
				success: function (data, status, jqxhr) {
					pageIndices[relatedVideoIDs.length]--;
					currPageTokens[relatedVideoIDs.length] = prevPageTokens[relatedVideoIDs.length];
					build(data);
...
function setJQuery() {
	...
	$('.navPic').mouseenter(function() {
		...
		// Find keyIndex of video picture
		var idString = $(this).attr('id');
		if (idString == 'lastPagePic' && pageIndices[relatedVideoIDs.length] > 1) keyIndex = 0;
