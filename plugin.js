...
function load() {
  ...
  for (var i = 1; i < parameters.length; i++) {
    try {
      ...
      // If parameter is string and string is treated as variable (otherwise undefined)
...
$(document).ready(function() {
  ...
  $("body").keydown(function(event) {
    ...
    else if (key == keys.LEFT || key == keys.ENTER || key == keys.DOWN)
      confirm();
    else {
      let e = event.originalEvent;
      let index = ["KeyP", "KeyR", "KeyV", "KeyB"].indexOf(e.code);
      if (index >= 0) {
        keyIndex = index;
        confirm();
      }
